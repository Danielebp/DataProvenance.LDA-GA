...
class LDAAccumulativeModel {
 public:
  ...
  // format.
  void AppendAsString(map<string, int>* word_index_map, std::ostream& out) const;
  string GetWordsPerTopic(map<string, int>* word_index_map, int topic, int nWords, string delimiter = ";") ;
 ...
 private:
  ...
  // The summation of P(word|topic) matrices
  // estimated by Gibbs sampling iterations after the burn-in period.
  vector<TopicProbDistribution> topic_distributions_;
  // The summation of P(topic|doc) matrices
  // estimated by Gibbs sampling iterations after the burn-in period.
  vector<TopicProbDistribution> doc_distributions_;
  // P(topic) vectors
  // estimated by Gibbs sampling iterations after the burn-in period.
